...
- Every MCP tool that resolves a resource class answers `status=index_not_ready` while the project indexes are still building, instead of `not_found` — "ask again" and "not here" are different answers. `bear_alps_transition_lookup` keeps answering from the profile and marks only the code side (`implementationsUnavailable`).
