...
- MCP tool `bear_app_context_list` returns the contexts the application actually boots under, read from the arguments its own entry points and tests pass, with the file and line each is written at.
- MCP tool `bear_di_module_tree_read` returns the tree of modules a context string installs, in Ray.Di's own priority order, with the file and line each `install()` is written at, and — with `diagram=true` — a Mermaid drawing of the same tree. A **BEAR Module Tree** tool window draws the same tree: the context is picked from the ones the application boots under, hovering a module shows the class and the file it is written in, and clicking one opens it.
- MCP tool `bear_aop_pointcut_lookup` returns which Ray.Aop interceptors wrap a class's methods, and the pointcut that binds each one, evaluated as Ray.Aop's own matchers evaluate it.
- MCP tool `bear_di_object_graph` returns what a class is actually built out of in one context — the object graph Ray.Di would assemble, resolved from the source rather than by booting anything — and, with `diagram=true`, a Mermaid drawing of the same graph.
...
- Body key completion answered with this project's resource for a URI naming another application (`app://payments/user`), and could answer from an interface, trait or enum declared before the resource class in the file — the two holes closed for the MCP tools in this release. It now applies the same rules, and no longer reads a schema from outside the project when `jsonSchemaPath` names a path that leaves it (#58).
- A `#[JsonSchema]` attribute naming only a key had that key read as its schema file: `#[JsonSchema(key: 'point')]` made body key completion, `bear_schema_lookup` and `bear_contract_compare` look for a file called `point`, find none, and — because the attribute looked like it had named one — not fall back to the conventional file either. An argument now counts towards a parameter's position only when it is written without a name (#58).
