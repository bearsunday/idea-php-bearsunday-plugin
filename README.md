...
* Read-only MCP tools that let an AI agent ask the IDE about resource classes, the attributes they carry and the interceptors bound to them, `$this->body` shapes, JSON Schema files, the generated OpenAPI document and ALPS profiles, and compare what each of those three layers names
