...
| Generate body JSON Schema | `demo-app/src/Resource/App/BodyTypeDemo.php` | Run **Generate BEAR body JSON Schema**; it writes `var/json_schema/body-type-demo.json` without a method name in the file. Body key completion reads that file whether or not a `#[JsonSchema]` attribute names it. |
...
| Body key completion without a `#[JsonSchema]` attribute | `demo-app/src/Resource/App/Point.php` | The class declares no schema attribute; `demo-app/var/json_schema/point.json` is named by the convention alone. Complete after `->body['` on `app://self/point` to get `x` and `y`. |
...
project's conventional methodless schema path, for example `var/json_schema/article.json`. Body key
completion falls back to that same path, so the generated file is offered after `->body['` without
a `#[JsonSchema]` attribute.
...
| `bear_schema_lookup` | The JSON Schema files of a resource, from the `#[JsonSchema]` attribute or the resource-class naming convention, looked for under every configured schema directory |
